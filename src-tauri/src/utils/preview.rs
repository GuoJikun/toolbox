...
pub fn preview_file(path: String) -> Result<File, String> {
    ...
    if let Some(file_type) = File::get_file_type(&path) {
        ...
        match matcher_type {
            MatcherType::Book => {
                File::book(path.clone(), String::from("Book"), extension.to_string())
            }
            MatcherType::Image => {
                File::image(path.clone(), String::from("Image"), extension.to_string())
            }
            MatcherType::Video => {
                File::video(path.clone(), String::from("Video"), extension.to_string())
            }
            MatcherType::Audio => {
                File::audio(path.clone(), String::from("Audio"), extension.to_string())
            }
            MatcherType::Text => {
                File::text(path.clone(), String::from("Text"), extension.to_string())
            }
            MatcherType::Doc => {
                File::docs(path.clone(), String::from("Doc"), extension.to_string())
            }
            MatcherType::App => File::app(path.clone(), String::from("App"), extension.to_string()),
            MatcherType::Archive => {
                File::archive(path.clone(), String::from("Archive"), extension.to_string())
            }
            MatcherType::Font => {
                File::font(path.clone(), String::from("Font"), extension.to_string())
            }
            MatcherType::Custom => {
                File::custom(path.clone(), String::from("other"), extension.to_string())
            }
        }
    } else {
